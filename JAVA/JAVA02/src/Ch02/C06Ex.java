...
class Profile{
	//속성 | getter setter를 만들기 위해private설정
	private String name;
	private String addr;
	private String job;
	...
	public Profile(String name, String addr, String job, String major) {
		super();				// Object의 생성자 호출, 사실 생략 가능
		this.name = name;		// 전달받은 name → 멤버변수 name
		this.addr = addr;		// 전달받은 addr → 멤버변수 addr
		this.job = job;			// 전달받은 job → 멤버변수 job
		this.major = major;		// 전달받은 major → 멤버변수 major
	}
	...
	boolean isEquals(String str) {
		...
		String params [] = str.split(",");//0, 1, 2, 3인덱스 까지보는데 이름이 다 맞아야함
		if(
